...
for k in ('100', '400', '1000'):
    ...
    FCds = np.zeros([Ndim*2, Ndim*2, N], dtype=float)
    SCds = np.zeros([Ndim*2, Ndim*2, N], dtype=float)  
    GDds = np.zeros([Ndim*2, Ndim*2, N], dtype=float)  
    MPCds = np.zeros([Ndim*2, Ndim*2, N], dtype=float)  
    
    for i, dataset in enumerate(DataSets):
        
        # Datasets Functional Connectomes
        if os.path.exists('FC/' + dataset):
            FCds[:,:,i] = np.mean(fg.load_mtxs('FC/' + dataset + '/', Ndim, fg.load_fc, atlas),  axis=2)
            
        # Datasets Structural Connectomes
        if os.path.exists('SC/' + dataset):
            SCds[:,:,i] = np.mean(fg.load_mtxs('SC/' + dataset + '/', Ndim, fg.load_sc, atlas),  axis=2)
    
        # Datasets Geodesic Distance Connectomes
        if os.path.exists('GD/' + dataset):
            GDds[:,:,i] = np.mean(fg.load_mtxs('GD/' + dataset + '/', Ndim, fg.load_gd, atlas),  axis=2)
    
        # Datasets MPC Connectomes
        if os.path.exists('MPC/' + dataset):
            MPCds[:,:,i] = np.mean(fg.load_mtxs('MPC/' + dataset + '/', Ndim, fg.load_mpc, atlas),  axis=2)
    
    # Group level correlations
    fg.dataset_corr(Means=SCds, Title='SC'+k, DataSets=DataSets)
    fg.dataset_corr(Means=FCds, Title='FC'+k, DataSets=DataSets)
    fg.dataset_corr(Means=GDds, Title='GD'+k, DataSets=DataSets)
    fg.dataset_corr(Means=MPCds, Title='MPC'+k, DataSets=DataSets)
