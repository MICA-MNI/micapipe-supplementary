...
for dataset in DataSets:
    if os.path.exists('FC/' + dataset):
        ...
        # ----------------------------------------------------------------------------
        FC = fg.load_mtxs('FC/' + dataset + '/', Ndim, fg.load_fc)
        ...
        corr_plot.figure.savefig(dataset + '_FC_mtx.png')
        ...
        for i, g in enumerate(FCgm.gradients_.T[0:10,:]):
            ...
            plot_hemispheres(fs5_lh, fs5_rh, array_name=grad[i], cmap='RdYlBu_r', nan_color=(0, 0, 0, 1),
                             ...
                             screenshot=True, filename=dataset + '_FC-G'+str(i+1)+'.png')   
        
        # MPC group correlation
        FCrho = fg.group_corr(Mtx=FC, group_gm=FCgm, corrplots=False, gmplots=False, Title='FC ', S=0.9)
        
        # save to csv file
        np.savetxt(dataset + '_FC-rho.csv', FCrho, delimiter=',')
        
    if os.path.exists('SC/' + dataset):
        # ----------------------------------------------------------------------------
        # STRUCTURAL GRADIENTS
        # ----------------------------------------------------------------------------
        SC = fg.load_mtxs('SC/' + dataset + '/', Ndim, fg.load_sc)
        ...
        corr_plot.figure.savefig(dataset + '_SC_mtx.png')
        ...
        for i, g in enumerate(SC_gradients.T[0:10,:]):
            ...
            plot_hemispheres(fs5_lh, fs5_rh, array_name=grad[i], cmap='RdYlBu_r', nan_color=(0, 0, 0, 1),
                             ...
                             color_bar='right', layout_style='grid', 
                             label_text={'left': ['Lateral', 'Medial'], 'top': ['Left', 'Right']},
                             screenshot=True, filename=dataset + '_SC-G'+str(i+1)+'.png')   
        ...
        # save to csv file
        np.savetxt(dataset + '_SC-rho.csv', SCrho, delimiter=',')
    ...
    if os.path.exists('GD/' + dataset):
        ...
        # ----------------------------------------------------------------------------
        GD = fg.load_mtxs('GD/' + dataset + '/', Ndim, fg.load_gd)
        ...
        corr_plot.figure.savefig(dataset + '_GD_mtx.png')
        ...
        for i, g in enumerate(GD_gradients.T[0:10,:]):
            ...
            plot_hemispheres(fs5_lh, fs5_rh, array_name=grad[i], cmap='RdYlBu_r', nan_color=(0, 0, 0, 1),
                                  ...
                                  color_bar='right', layout_style='grid', 
                                  label_text={'left': ['Lateral', 'Medial'], 'top': ['Left', 'Right']},
                                  screenshot=True, filename=dataset + '_GD-G'+str(i+1)+'.png')
        ...
        # save to csv file
        np.savetxt(dataset + '_GD-rho.csv', GDrho, delimiter=',')
        
    if os.path.exists('MPC/' + dataset):
        # ----------------------------------------------------------------------------
        # MPC GRADIENTS
        # ----------------------------------------------------------------------------
        MPC = fg.load_mtxs('MPC/' + dataset + '/', Ndim, fg.load_mpc)
        # Mean matrix across the z axis
        MPCmean = np.mean(MPC, axis=2)
        
        # Plot the log matrix
        corr_plot = plotting.plot_matrix(MPCmean, figure=(10, 10), labels=None, cmap='Greens')
        corr_plot.figure.savefig(dataset + '_MPC_mtx.png')
        ...
        for i, g in enumerate(MPCgm.gradients_.T[0:10,:]):
            ...
            plot_hemispheres(fs5_lh, fs5_rh, array_name=grad[i], cmap='RdYlBu_r', nan_color=(0, 0, 0, 1),
                                  ...
                                  color_bar='right', layout_style='grid', 
                                  label_text={'left': ['Lateral', 'Medial'], 'top': ['Left', 'Right']},
                                  screenshot=True, filename=dataset + '_MPC-G'+str(i+1)+'.png')    
        
        # MPC group correlation
        MPCrho = fg.group_corr(Mtx=MPC, group_gm=MPCgm, corrplots=False, gmplots=False, Title='MPC', S=0.9)
        
        # save to csv file
        np.savetxt(dataset + '_MPC-rho.csv', MPCrho, delimiter=',')
